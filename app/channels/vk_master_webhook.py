"""VK Callback API parse/auth for master private dialog (CURSOR-29)."""
...
__all__ = ("parse_vk_master_callback",)
...
def parse_vk_master_callback(
    ...
    *,
    ...
    try:
        ...
        return VkMasterWebhookResult(kind=VkMasterWebhookKind.REJECTED)
    ...
    if event_type == "confirmation":
        ...
        return VkMasterWebhookResult(
            kind=VkMasterWebhookKind.CONFIRMATION,
            confirmation_response=config.confirmation,
        )

    if event_type != "message_new":
        return VkMasterWebhookResult(kind=VkMasterWebhookKind.IGNORED)

    message = _extract_private_message(payload.get("object"), group_id=group_id)
    if message is None:
        return VkMasterWebhookResult(kind=VkMasterWebhookKind.IGNORED)
    return VkMasterWebhookResult(kind=VkMasterWebhookKind.MESSAGE, message=message)
...
def _extract_private_message(
    ...
    *,
    group_id: int,
) -> VkMasterNormalizedMessage | None:
    if type(obj) is not dict:
        return None
    message = obj.get("message")
    if type(message) is not dict:
        # Older Callback shapes nested differently — fail closed / ignore.
        return None
    ...
    try:
        return VkMasterNormalizedMessage(
            from_id=from_id,
            peer_id=peer_id,
            text=cleaned,
