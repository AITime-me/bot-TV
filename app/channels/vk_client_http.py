...
configured. ACK only after durable MESSAGE receipt. No MasterCommand / outbound.
"""
...
from app.channels.vk_client_types import VkClientNormalizedMessage, VkClientWebhookKind
...
async def handle_vk_client_callback(
    ...
    *,
    ...
    try:
        ...
        # Defense in depth: never re-raise (Pydantic may embed raw input_value).
        # Oversized/control text is already IGNORED at parse; this is unreachable
        # for the normal Callback path.
        return VkClientWebhookHttpResult(body="ok")
