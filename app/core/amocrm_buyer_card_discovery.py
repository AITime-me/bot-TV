"""amoCRM Buyer Card (Customer) discovery result types (IR-3).

Read-only Customer-candidate evidence for IdentityResolutionService.reconcile_buyer_card.
No Lead status. No attach/create. No PII in repr.
"""
...
class AmoCrmBuyerCardDiscoveryResult:
    """Discovery receipt. Customer/contact ids are technical amoCRM ids only."""
    ...
    def __post_init__(self) -> None:
        if self.outcome is AmoCrmBuyerCardDiscoveryOutcome.FOUND_CANDIDATE:
            ...
            if len(self.eligible_customer_ids) != 1:
            ...
            if len(self.eligible_customer_ids) < 2:
            ...
            if len(self.eligible_customer_ids) != 0:
                raise TypeError("NOT_FOUND must not carry eligible ids") from None
        elif self.eligible_customer_ids:
            raise TypeError(
                f"{self.outcome.value} must not carry eligible_customer_ids"
            ) from None
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"eligible_customer_ids={self.eligible_customer_ids!r}, "
...
def buyer_card_reconcile_candidates_from_discovery(
    ...
    """Map a complete Customer discovery into reconcile kwargs. Never attaches."""

    if result.outcome not in _COMPLETE_OUTCOMES:
        return None
    return BuyerCardReconcileCandidates(
        candidate_buyer_card_ids=result.eligible_customer_ids,
    )
