...
async def truncate_foundation_tables(
    ...
    async with session_factory() as session:
        async with session.begin():
            ...
            has_self_booking_active_offer = await session.scalar(
                text(
                    "SELECT to_regclass('public.self_booking_active_offers') "
                    "IS NOT NULL"
                )
            )
            has_self_booking_pii_admission = await session.scalar(
                text(
            ...
            has_booking_method_pendings = await session.scalar(
                text(
                    "SELECT to_regclass("
                    "'public.booking_method_analytics_pendings') "
                    "IS NOT NULL"
                )
            )
            ...
            if has_self_booking_active_offer:
                tables.insert(0, "self_booking_active_offers")
            if has_self_booking_pii_admission:
                tables.insert(0, "self_booking_pii_admissions")
